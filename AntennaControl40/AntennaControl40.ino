...
#define MaxSpeedAZ 2000
#define MaxAccelAZ 400
#define MaxSpeedEL 2000
#define MaxAccelEL 400
...
const int AZResolutionPerTurn = 4000;  // Stepper counts to make one revolution of the Axis.
const int ELResolutionPerTurn = 3200;  // Stepper counts to make one revolution of the Axis.
...
AccelStepper ELmotor(1, 2, 3); // 1=Driver, 2=StepPin, 3=DirPin
AccelStepper AZmotor(1, 5, 6); // 1=Driver, 5=StepPin, 6=DirPin
...
void setup() {
  ...
  ELmotor.setEnablePin(4);
  AZmotor.setEnablePin(7);
...
void AZfindhome() {
  ...
  AZmotor.runToNewPosition(0); // Will this unwind the cables.
  // First move away from the flag in case it's close.
  for (int i = 0; i <= AZResolutionPerTurn / 8; i++) AZforward();
  long timeout = 0;
...
void ELfindhome() {
  ...
  if (!digitalRead(ELhomePin))  // Are we at the flag. Move off to find home.
    for (int i = 0; i <= 12250; i++)
      ELmotor.move(100);
      //ELbackward();ELmotor.move(1);
  long timeout = 0;

ELmotor.moveTo(4000);
/*ELmotor.run();
  while (digitalRead(ELhomePin) && (timeout < ELResolutionPerTurn / 2))
  {
    timeout++;  // Don't just move forever, stop after a resonable time.
    ELforward();
  }
 while ((timeout < ELResolutionPerTurn / 2))
  {
   // timeout++;  // Don't just move forever, stop after a resonable time.
    ELforward();
  }
...
ELmotor.disableOutputs();  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx
*/
  Serial.println("Done.");
}
...
void TakeShortCut(int RequestedSteps) {
  if ((AZmotor.currentPosition() - RequestedSteps) > AZResolutionPerTurn / 2) {
  ...
  if (minute() >= CalibrationTimeout) {  // After this many minutes, timout and do calibration and Home.
    ...
    */
    ...
    if (needhomed) {
      ...
      ELmotor.runToNewPosition(0);  // Blocks and drives back to the zero position.
