...
void AZforward() {  
  AZmotor->onestep(BACKWARD, DOUBLE);
}
void AZbackward() {  
  AZmotor->onestep(FORWARD, DOUBLE);
}
// wrappers for the second motor!
void ELforward() {  
  ELmotor->onestep(BACKWARD, DOUBLE);
}
void ELbackward() {  
  ELmotor->onestep(FORWARD, DOUBLE);
}
...
void ELfindhome() {
  ...
  if (!digitalRead(ELhomePin))  // Are we at the flag. Move off to find home.
    for (int i = 0; i <= 250; i++) ELmotor->step(1, BACKWARD, INTERLEAVE);
  long timeout = 0;
  while (digitalRead(ELhomePin) && (timeout < ELResolutionPerTurn / 2)) {
    timeout++;  // Don't just move forever, stop after a resonable time.
    ELmotor->step(1, FORWARD, INTERLEAVE); // Move to the flag.
  }
