...
AccelStepper ELmotor(1, 31, 33); // 1=Driver, 2=StepPin, 3=DirPin
AccelStepper AZmotor(1, 39, 41); // 1=Driver, 5=StepPin, 6=DirPin
...
void AZfindhome() {
  ...
  long homecount = AZCommandPosition;
  ...
  {
    homecount = -AZResolutionPerTurn/8;
...
void ProcessCommands()  { // We got any new commands
  ...
  switch (MessageNum) {
    ...
    default:
      ...
      Serial.print("New Coords: "); Serial.println(inputString);
      ...
      if (divider >= 1) {
        ...
        AZCommandPosition = -NewNumber;  // Negative to reverse movement
...
void loop() {
  ...
  if (AZmotor.distanceToGo() == 0) AZmotor.disableOutputs(); else AZmotor.enableOutputs();
  if (ELmotor.distanceToGo() == 0) ELmotor.disableOutputs(); else ELmotor.enableOutputs();
